...
Las trayectorias se generan con la biblioteca \texttt{andi-datasets}, desarrollada para reproducir los modelos y conjuntos del reto AnDi \cite{andidatasets2021}. El código utilizado para generar los datos sintéticos, entrenar los modelos y reproducir los análisis se encuentra disponible en el repositorio asociado del proyecto \cite{chair2026repo}. Para garantizar la trazabilidad, cada ejecución debe registrar la versión de la biblioteca, las dependencias, las semillas y el \emph{commit} exacto del repositorio.
...
La reproducibilidad del estudio se apoya en la conservación conjunta de los cuadernos experimentales, los scripts de generación, las configuraciones utilizadas y los resultados agregados. Para reproducir exactamente una ejecución concreta deben registrarse también las semillas, las versiones de las dependencias y el \emph{commit} utilizado.
...
\paragraph{Validez del generador.} Modelo y exponente cambian simultáneamente; los segmentos se normalizan; y el conjunto es sintético. La red puede aprender regularidades específicas del simulador o artefactos de concatenación. Por ello, las conclusiones se restringen al generador, los rangos paramétricos y las transformaciones documentadas en este estudio.
...
La reproducibilidad no depende solo de publicar métricas finales, sino de conservar el código, los cuadernos, las configuraciones, las semillas, las versiones de las dependencias y el \emph{commit} del repositorio asociado \cite{chair2026repo}. También deben registrarse el dispositivo, los comandos de generación y entrenamiento y los resultados agregados. Estos elementos permiten distinguir con claridad los resultados obtenidos de las extensiones que permanecen como trabajo futuro. El repositorio constituye una fuente de implementación y trazabilidad, pero no sustituye las referencias científicas.
...
El siguiente esquema resume la construcción de las particiones sin reproducir detalles dependientes de una implementación concreta. El conjunto de modelos es
\[
  \Mset=\{\mathrm{ATTM},\mathrm{CTRW},\mathrm{FBM},\mathrm{LW},\mathrm{SBM}\},
\]
y $L_{\min}$ denota la longitud mínima permitida para cada segmento.
