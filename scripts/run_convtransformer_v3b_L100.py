...
"""
Run ConvTransformer-v3b for L=100 on validation only.
...
The only methodological change introduced here is the input representation:
    ...
    ConvTransformer-v3b: (N, 99, 6) + soft localization labels
...
to:
    ...
    results/convtransformer_v3_ablation/v3b_seed42/validation_metrics.csv
    results/convtransformer_v3_ablation/v3b_seed42/config_used.yaml

Scientific note:
ConvTransformer-v3b mantiene la entrada multicanal de v3a e introduce
etiquetas suaves de localización para comprobar en validación si la supervisión
gradual mejora el compromiso entre detección y localización.
"""
...
from scripts.label_engineering import build_soft_cp_labels  # noqa: E402
...
@tf.keras.utils.register_keras_serializable(package="Changepoint")
def soft_ce_localization(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
    """Categorical cross-entropy for soft changepoint-position labels."""
    y_true = tf.cast(y_true, tf.float32)
    y_pred = tf.clip_by_value(tf.cast(y_pred, tf.float32), 1e-7, 1.0 - 1e-7)
    y_true = y_true / tf.maximum(tf.reduce_sum(y_true, axis=-1, keepdims=True), 1e-7)
...
def build_convtransformer_v3b_model(
    ...
    return models.Model(inputs=inputs, outputs=[has_cp, cp_dist], name="convtransformer_v3b_L100")
...
def compile_model(model: models.Model) -> models.Model:
    ...
    model.compile(
        ...
        loss=[weighted_binary_crossentropy_no_cp, soft_ce_localization],
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train ConvTransformer-v3b L=100 and evaluate validation only.")
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "By default, full runs use v3b_seed42 and reduced runs use v3b_seed42_fast."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Gaussian sigma, in time points, for v3b soft localization labels.",
    )
    parser.add_argument(
        "--max-train-samples",
        type=int,
...
def main() -> None:
    """Run the validation-only ConvTransformer-v3b experiment."""
    args = parse_args()
    set_global_seed(args.seed)

    reduced_run = args.max_train_samples is not None or args.max_val_samples is not None
    if args.output_dir is None:
        run_name = f"v3b_seed{args.seed}_fast" if reduced_run else f"v3b_seed{args.seed}"
    ...
    config_used = {
        "experiment_name": "convtransformer_v3b_L100",
        "ablation_version": "v3b",
        "status": "validation_only_no_test_results",
        "scientific_note": (
            "ConvTransformer-v3b mantiene la entrada multicanal de v3a e introduce etiquetas suaves "
            "de localizacion para evaluar en validacion si la supervision gradual mejora el compromiso "
            "entre deteccion y localizacion."
        ),
        "seed": args.seed,
        "length": LENGTH,
        "dx_length": DX_LENGTH,
        ...
        "soft_labels": {
            "enabled": True,
            "sigma": args.soft_label_sigma,
            "length": DX_LENGTH,
            "valid_min": VALID_DX_MIN,
            "valid_max": VALID_DX_MAX,
            "no_changepoint_distribution": "uniform_masked_by_zero_sample_weight",
        },
        "training": {
    ...
    print("Building soft localization labels...")
    y_cp_soft_train = build_soft_cp_labels(
        y_cp_train,
        y_has_train,
        length=DX_LENGTH,
        sigma=args.soft_label_sigma,
    ...
    y_cp_soft_val = build_soft_cp_labels(
        y_cp_val,
        y_has_val,
        length=DX_LENGTH,
        sigma=args.soft_label_sigma,
    ...
    train_ds = make_dataset(x_train, y_has_train, y_cp_soft_train, batch_size=args.batch_size, training=True)
    val_ds = make_dataset(x_val, y_has_val, y_cp_soft_val, batch_size=args.batch_size, training=False)

    model = build_convtransformer_v3b_model(input_shape=(DX_LENGTH, N_INPUT_CHANNELS))
    compile_model(model)
    model.summary()
    ...
    print("Starting validation-only v3b training protocol.")
    ...
    validation_metrics["experiment_name"] = "convtransformer_v3b_L100"
    validation_metrics["soft_label_sigma"] = args.soft_label_sigma
